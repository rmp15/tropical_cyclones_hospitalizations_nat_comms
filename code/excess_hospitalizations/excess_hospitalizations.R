...
dat.admissions = subset(dat.admissions, year==end_year & month%in%c(5:10))
...
dat.complete.subset = subset(dat.complete, year==end_year & month%in%c(5:10))

# find average number of hospitalizations for the entire hurricane season (could do loop but lazy)
dat.admissons.css.level.1.mean = ddply(dat.complete.subset,.(year,fipscounty,ccs_level_1_description),summarize,cases.mean=mean(cases))
...
dat.admissions.ccs.level.1.mean.total$cases.additional = with(dat.admissions.ccs.level.1.mean.total, cases.mean * (rr-1))
...
dat.admissons.css.level.1.mean.int.lag$cases.additional = with(dat.admissons.css.level.1.mean.int.lag, cases.mean * (rr-1))
dat.admissons.css.level.1.mean.int.lag$cases.additional.ll = with(dat.admissons.css.level.1.mean.int.lag, cases.mean * (rr.ll-1))
dat.admissons.css.level.1.mean.int.lag$cases.additional.ul = with(dat.admissons.css.level.1.mean.int.lag, cases.mean * (rr.ul-1))
...
dat.admissions.ccs.level.1.mean.total.cause.summary.int.lag = ddply(dat.admissons.css.level.1.mean.int.lag,.(ccs_level_1_description),summarize,
                                                            cases.additional=sum(cases.additional),cases.additional.ll=sum(cases.additional.ll),
                                                            cases.additional.ul=sum(cases.additional.ul))

# PLOTS
dir.output = paste0('~/git/rmparks_coastal_storms_Jan_2020/figures/excess_hospitalizations/',start_year,'_',end_year,'/')
...
grid.arrange(p3,p4,nrow=2,left=paste("Additional hospitalization associated with additional day of\nstorm or hurricane exposure (based on 2014 population)"))
