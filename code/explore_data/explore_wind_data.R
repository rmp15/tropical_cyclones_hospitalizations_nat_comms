...
# output directory
dir.output = paste0('~/git/rmparks_coastal_storms_Jan_2020/figures/explore_wind_data/',start_year,'_',end_year,'/')
...
# how many events per county during period of study?
...
# plot for K99
pdf(paste0(dir.output,'map_number_days_exposed_for_K99_',start_year,'_',end_year,'.pdf'))#,paper='a4r',width=0,height=0)
print(ggplot() +
    geom_polygon(data=subset(USA.df.test),aes(x=long,y=lat,group=group,fill=cases),color='black',size=0.04) +
    # geom_polygon(data=subset(us_aea),aes(x=long,y=lat,group=group),fill=NA,color='black',size=0.2) +
    guides(fill = guide_colorbar(direction = "horizontal", title.position="left",barwidth = 10, barheight = 1,title.vjust = 0.8,
    ...
    theme(panel.grid.major = element_blank(), text = element_text(size = 20),
        axis.text.x = element_text(angle=90), axis.ticks.x=element_blank(),legend.text=element_text(size=100),
        panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"),
        panel.border = element_rect(colour = "black"),strip.background = element_blank(),legend.justification='center',
        legend.position = 'bottom', legend.background = element_rect(fill="white", size=.5, linetype="dotted")) +
    theme_map(20)
