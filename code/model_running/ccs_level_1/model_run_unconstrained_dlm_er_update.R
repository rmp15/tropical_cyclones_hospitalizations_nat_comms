...
# create expanded list of CCS Level 1 group and er/non-er
expanded.grid = expand.grid(labels=labels,causes=causes_groups)

# extract label and cause from seedvalue
chosen.row = expanded.grid[seedVal,]
label=as.character(chosen.row[,1])
causes_group = as.character(chosen.row[,2])
...
print(paste0(causes_group,' ',label))
...
input.file = paste0(dir.input,'medicare_',gsub(" ", "_", causes_group),'_rates_',label,'_expanded_grid_hospitalisations_',years[1],'_',years[length(years)],'.rds')
...
if(!file.exists(input.file)){
    ...
    print('data not processed... stopping now...')
...
saveRDS(mod_dlm_unconstrained, paste0(dir.output.model.output,'medicare_',causes_group,'_',label,'_model_summary_update_',years[1],'_',years[length(years)],'.rds'))

# save w temperature model summary
write.csv(dat.results, paste0(dir.output.model.summary,'medicare_',causes_group,'_',label,'_model_summary_update_',years[1],'_',years[length(years)],'.csv'))

# save wo temperature model summary
write.csv(dat.results_no_temp, paste0(dir.output.model.summary,'medicare_',causes_group,'_',label,'_model_summary_update_no_temp_',years[1],'_',years[length(years)],'.csv'))

library(ggplot2)

pdf(paste0(dir.output.plots,'medicare_rr_values_model_summary_unconstained_dlm_update_',causes_group,'_',label,'_',years[1],'_',years[length(years)],'.pdf'),paper='a4r',height=0,width=0)
print(
