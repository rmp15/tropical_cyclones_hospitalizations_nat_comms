...
cod = c('Miscellaneous mental health disorders    ', 'Aspiration pneumonitis; food/vomitus')
...
dat.results_no_temp$cause = gsub("_", " ", cod.arg)
...
pdf(paste0(dir.output.plots,'medicare_rr_values_model_summary_unconstained_dlm_update_',cod.arg,'_',years[1],'_',years[length(years)],'.pdf'),paper='a4r',height=0,width=0)
print(
...
saveRDS(mod_dlm_unconstrained, paste0(dir.output.model.output,'medicare_',gsub(" ", "_", cod.arg),'_model_summary_update_',years[1],'_',years[length(years)],'.rds'))

# save w temperature model summary
write.csv(dat.results, paste0(dir.output.model.summary,'medicare_',gsub(" ", "_", cod.arg),'_model_summary_update_',years[1],'_',years[length(years)],'.csv'))

# save wo temperature model summary
write.csv(dat.results_no_temp, paste0(dir.output.model.summary,'medicare_',gsub(" ", "_", cod.arg),'_model_summary_update_no_temp_update_',years[1],'_',years[length(years)],'.csv'))
