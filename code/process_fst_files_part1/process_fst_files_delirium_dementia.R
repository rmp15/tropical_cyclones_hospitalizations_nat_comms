...
if(year%in%c(1999:2014)){
    ...
    setwd('~/shared_space/ci3_analysis/rmparks_coastal_storms_Jan_2020/data/Single_Level_CCS_2015/')
...
# divide up by delirium and dementia
codes = c(2900, 29010, 29011, 29012, 29013, 29020, 29021, 2903, 29040, 29041, 29042, 29043, 2908, 2909, 2930, 2931, 2940, 2941,
        29410, 29411, 29420, 29421, 2948, 2949, 3100, 3102, 3108, 31081, 31089, 3109, 3310, 3311, 33111, 33119, 3312, 33182, 797)
css_category_2 = c('dementia', 'dementia', 'dementia with delirium', 'dementia', 'dementia', 'dementia', 'dementia',
                   'dementia with delirium', 'dementia', 'dementia with delirium', 'dementia', 'dementia', 'other', 'other',
                    'delirium', 'delirium', 'other', 'dementia', 'dementia', 'dementia', 'dementia', 'dementia', 'other',
                    'other', 'other', 'other', 'other', 'other', 'other', 'other', 'dementia', 'dementia', 'dementia',
                    'dementia', 'dementia', 'dementia', 'other')

dat.653 = data.frame(codes=codes, css_category_2=css_category_2)
...
# for(i in c(1:2)){
    ...
    # filter for delirium/dementia
    dat_admissions_sum = subset(dat_admissions_sum,css_category=='653') # 653 is dementia/delirium
    print(head(dat_admissions_sum))

    # match up the cause of hospitalization for dementia/delirium with the DIAG1 coding
    dat_admissions_sum = merge(dat_admissions_sum,dat.653,by.x=c('DIAG1'),by.y=c('codes'),all.x=TRUE)
    ...
    dat_admissions_sum = ddply(dat_admissions_sum,.(SSA_STATE_CD,SSA_CNTY_CD,YEAR,ADATE,css_category_2),summarise,cases=sum(cases))
...
dat_admissions_sum_total = ddply(dat_admissions_sum_total,.(SSA_STATE_CD,SSA_CNTY_CD,YEAR,ADATE,css_category_2),summarise,cases=sum(cases))
...
saveRDS(dat_admissions_sum_total, paste0(dir.output,'medicare_admissions_dementia_delirium_processing_',year,'.rds'))
...
write.csv(dat_admissions_sum_total_na, paste0(dir.output,'NA_medicare_admissions_dementia_delirium_processing_',year,'.csv'))