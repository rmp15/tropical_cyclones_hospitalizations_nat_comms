...
# load results
dat.results <- read.csv(paste0(data.folder,'figure_5_model_results_',start_year,'_',end_year,'.csv'))
...
pdf(paste0(output.folder,'figure_5.pdf'),paper='a4r',width=0,height=0)
ggplot() +
    geom_errorbar(data=subset(dat.results),aes(x=as.factor(lag.factor),ymax=rr.ll.bfc-1,ymin=rr.ul.bfc-1,color=as.factor(event)),width=0.5,size=0.5,alpha=1,position=position_dodge(width=0.7)) +
    geom_point(data=subset(dat.results), aes(x=as.factor(lag.factor),y=rr-1,color=as.factor(event)),size=2.5,shape=16,position=position_dodge(width=0.7)) +
