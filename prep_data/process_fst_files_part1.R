...
if(year%in%c(2015:2020)){
    # load crosswalk
    ccs_codes = readRDS(paste0(project.folder,'data/ccs_icd_crosswalk/ccs_icd_crosswalk.rds'))
}
...
# for(i in c(1:2)){
    ...
    # match up the cause of death groupings with the DIAG1 coding
    ...
    dat_admissions_sum = ddply(dat_admissions_sum,.(SSA_STATE_CD,SSA_CNTY_CD,YEAR,ADATE,css_category),summarise,cases=sum(cases))
...
dat_admissions_sum_total = ddply(dat_admissions_sum_total,.(SSA_STATE_CD,SSA_CNTY_CD,YEAR,ADATE,css_category),summarise,cases=sum(cases))
