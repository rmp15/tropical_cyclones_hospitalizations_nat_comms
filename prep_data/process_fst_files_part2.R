...
# for(year in c(1999:2004)){
    ...
    library(plyr)
    dat.current = ddply(dat.current,.(SSA_STATE_CD,SSA_CNTY_CD,css_category,day,month,year),summarise,cases=sum(cases))

    dat.all=rbind(dat.all,dat.current)
}

# summarise one final time to get over entire multi-year period
dat_admissions_sum_total = ddply(dat.all,.(SSA_STATE_CD, SSA_CNTY_CD,css_category,day,month,year),summarise,cases=sum(cases))

#reorder
dat_admissions_sum_total = dat_admissions_sum_total[order(dat_admissions_sum_total$SSA_STATE_CD,dat_admissions_sum_total$SSA_CNTY_CD,dat_admissions_sum_total$css_category,dat_admissions_sum_total$year,dat_admissions_sum_total$month,dat_admissions_sum_total$day),]
